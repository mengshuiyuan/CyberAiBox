...
private:
    ...
    void InitializeButtons()
    {
        ...
        boot_button_.OnClick([this]()
            ...
            app.WakeWordInvoke(Lang::Strings::HELLO_ARE_YOU_HERE);//"你好，在吗"
            app.ToggleChatState(); });
        boot_button_.OnDoubleClick([this]()
                                   {
                                       auto &app = Application::GetInstance();
                                       if (app.GetDeviceState() == kDeviceStateStarting || app.GetDeviceState() == kDeviceStateWifiConfiguring || app.GetDeviceState() == kDeviceStateActivating)
                                       {
                                           SwitchNetworkType();
                                       }
                                       if (GetNetworkType() == NetworkType::WIFI)
    ...
    static void ImuMonitoringTask(void *arg)
    {
        ...
        while (true)
        {
            ...
            vTaskDelay(pdMS_TO_TICKS(50)); // 50Hz采样率
        }
    }
    ...
    void NotifyShakeEvent(ShakeEventType type)
    {
        ...
        case SHAKE_DETECTED:
            ...
            {
                app.TriggerWakeWord(Lang::Strings::SHAKE); //"摇晃你,你怎么办"
                vTaskDelay(pdMS_TO_TICKS(1000));
                display->SetEmotion("confused");
            }
            else if (app.GetDeviceState() == kDeviceStateIdle)
            {
                app.TriggerWakeWord(Lang::Strings::SHAKE_AWAKE); //"晃醒你,你怎么办"
                vTaskDelay(pdMS_TO_TICKS(1000));
                display->SetEmotion("confused");
            }
            break;
...
public:
    CyberAiDualScreen() : DualNetworkBoard(ML307_TX_PIN, ML307_RX_PIN, GPIO_NUM_NC, (int32_t)NetworkType::ML307), // GPIO_NUM_NC
                          boot_button_(BOOT_BUTTON_GPIO)
    { //,touch_button_(TOUCH_BUTTON)
        InitializeI2c();
