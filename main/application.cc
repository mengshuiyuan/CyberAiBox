...
namespace {
constexpr float kStandardGravityMps2 = 9.807f;
constexpr float kShakeThresholdMps2 = 2.0f;
constexpr float kShakeAxisThresholdMps2 = 10.5f;
constexpr uint32_t kShakeCheckIntervalSeconds = 2;
} // namespace

volatile bool is_confused = false;
...
void Application::Start() {
    ...
    protocol_->OnAudioChannelOpened([this, codec, &board]() {
        ...
        if (protocol_->server_sample_rate() != codec->output_sample_rate()) {
            ...
            board.GetMusic()->ResetSampleRate();
        }
    });
    ...
    auto dual_board = dynamic_cast<DualNetworkBoard*>(&board);
 if(dual_board->GetBoardType() == "ml307")
    {
        //TriggerWakeWord("已进入4G模式");
        audio_service_.PlaySound(Lang::Sounds::OGG_SPEAK_4G);
    }
    else
    {
        //TriggerWakeWord("已进入WiFi模式");
...
void Application::MainEventLoop() {
    static auto display = Board::GetInstance().GetDisplay();
    static uint32_t shake_check_accumulator = 0;
    while (true) {
        ...
        if (bits & MAIN_EVENT_CLOCK_TICK) {
            ...
            //ESP_LOGI(TAG, "GPIO47 level: %d", gpio_get_level(GPIO_NUM_47));
            // shake_check_accumulator++;
            // bool ready;
            // if (shake_check_accumulator >= kShakeCheckIntervalSeconds) {
            //     shake_check_accumulator = 0;
            //     float ax = 0.0f, ay = 0.0f, az = 0.0f;
            //     if (Board::GetInstance().ReadAccelerometer(ax, ay, az)) {
            //          ESP_LOGI(TAG, "IMU Acceleration: ax=%.2f ay=%.2f az=%.2f", ax, ay, az);
            //         const float magnitude = std::sqrt(ax * ax + ay * ay + az * az);
            //         const float delta = std::fabs(magnitude - kStandardGravityMps2);
            //         const float axis_peak = std::max({std::fabs(ax), std::fabs(ay), std::fabs(az)});
            //         if (delta > kShakeThresholdMps2 || axis_peak > kShakeAxisThresholdMps2) {
            //             ESP_LOGI(TAG, "Shake detected Δ=%.2f m/s^2 (|a|=%.2f) axis_peak=%.2f",
            //                      delta, magnitude, axis_peak);
            //             if(device_state_ == kDeviceStateListening||device_state_ == kDeviceStateSpeaking) 
            //             {
            //                 TriggerWakeWord("摇晃你,你怎么办");
            ...
            if (clock_ticks_ % 10 == 0) {
                ...
                //ESP_LOGI(TAG,"触摸2引脚电平: %d", gpio_get_level(GPIO_NUM_2));
                //
                // ESP_LOGI(TAG,"4G 14引脚电平: %d", gpio_get_level(GPIO_NUM_14));
                //ESP_LOGI(TAG,"11引脚电平 : %d", gpio_get_level(GPIO_NUM_11));  
                //  if(gpio_get_level(GPIO_NUM_14) == 0)
                //  {
                //     gpio_set_level(GPIO_NUM_14, 1);
                //     ESP_LOGI(TAG,"4G 14引脚电平软件拉高后 : %d", gpio_get_level(GPIO_NUM_14));
                //  }
                SystemInfo::PrintHeapStats();
            }
        }
