        ...
        if (low_battery_popup_ != nullptr) {
            if (battery_level < 10 ) {
                if (lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) { // 如果低电量提示框隐藏，则显示
                    ...
                    Application::GetInstance().TriggerWakeWord("电量低于百分之十,请及时充电");
                    //display_->ShowLowPower(true);
                    vTaskDelay(pdMS_TO_TICKS(5000)); 
                    gpio_config_t poweroff_config = {//关机
                    ...
                    ESP_ERROR_CHECK(gpio_config(&poweroff_config));
